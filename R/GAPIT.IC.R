...
`GAPIT.IC` <- function(DP=NULL){
  ...
  PC = DP$PC
  CV = DP$CV # Covariates
  GD = DP$GD # Genetic data
#  noCV = FALSE # Does not appear to be used.
  
  # If we receive no covariates, create a vector of 1s from Y or GD.
  if( is.null(CV) ){
#    noCV=TRUE
    if( ncol(GD) == 0 ){
      CV=Y[,1:2]
  ...
  # If we have PCA but no CV, create CV from PCA.
  if( DP$PCA.total > 0 & is.null(DP$CV) ){
    CV = PC
  }
  
  # If we have no GD but do have KI.
  ...
  if( nrow(comCV) != length(GT) ){
    stop ("GAPIT says: The number of individuals in CV does not match to the number of individuals in genotype files.")
  }
  ...
  if(DP$kinship.algorithm%in%c("FarmCPU","Blink","MLMM")){ 
    return (
      list( Y=comY, GT=GT, PCA=comCV, K=DP$KI, GD=comGD, GM=DP$GM, myallCV=CV, myallGD=GD )
    )
  }else{
    return (
      list( Y=comY, GT=GT, PCA=comCV, K=DP$KI, GD=comGD, GM=DP$GM, myallCV=CV, myallGD=GD, myallY=Y)
    )
  }
