`GAPIT.All` <- function(
  ...
  buspred=FALSE,lmpred=FALSE,FDRcut=FALSE,
  ...
  QC=TRUE,GTindex=NULL,LD=0.1,plot.bin=10^5,
  file.output=FALSE,cutOff=0.05, 
  Model.selection = FALSE, output.numerical = FALSE,
  output.hapmap = FALSE, Create.indicator = FALSE,Multi_iter=FALSE,
  num_regwas=10,opt="extBIC",
  QTN=NULL, QTN.round=1,QTN.limit=0, QTN.update=TRUE, QTN.method="Penalty",
  Major.allele.zero = FALSE,Random.model=FALSE,
  method.GLM="FarmCPU.LM",method.sub="reward",method.sub.final="reward",
  ...
  alpha=c(.01,.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,1),maxOut=100,
  ...
  if(!is.null(KI) & 
     is.null(G) & is.null(file.G) & 
     is.null(file.GD) & is.null(GD)){
    SNP.test = FALSE
  }
  model_store=model
  KI0=KI
