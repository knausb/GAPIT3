...
`GAPIT.Genotype.View` <-function(
  ...
  w1_end = NULL,
  mav1 = NULL){
# Object: Analysis for Genotype data:Distribution of SNP density,
#  Accumulation, Moving Average of density,
#  result: a pdf of the scree plot
# myGI, (= GM) a data.frame with column names "SNP", "Chromosome", and "Position".
# myGD, data.frame of genetic data in 'numerical' format, samples in rows, variants in columns, first column is taxa.
#  
# myG:Genotype data
...
##if(is.null(w1_end)){w1_end=100}
  ...
  rm(X, H, het.ind, het.snp) #Free memory
...
#print(dim(myFig3))
  ...
  save_Density_Cor <- matrix( data = 0.0,
                              nrow = sum_number_Density,
                              ncol = 3 )
  save_Density_Cor_name <- matrix( data = "",
                                   nrow = sum_number_Density,
                                   ncol = 1 )
...
#save_Density_Cor[countSDC,3]<-as.numeric(as.matrix(format(cor(myGD3_100[,j],myGD3_100[,k])%*% cor(myGD3_100[,j],myGD3_100[,k]),digits=8)))
      ...
      save_Density_Cor_name[ countSDC, 1 ] <- paste( results3_100[ j, 1 ],
                                                     "::::",
                                                     results3_100[k,1],
                                                     seq="")
      countSDC <- countSDC + 1
...
##mav1<-100
  ...
  result_mav2 <- matrix( data = 0.0,
                         nrow = sum_number_Density - mav1,
                         ncol = 1)
...
result_mav<-cbind(result_mav1,result_mav2)
  ...
  grDevices::pdf("GAPIT.Marker.LD.pdf",
                 width =10,
                 height = 6)
  graphics::par(mar = c(5,5,5,5))

  plot( as.matrix( result3_3[,1] ),
...
#,ylim=c(0,round(max(result3_3[,3]))))
  ...
  graphics::lines(result_mav[,2] ~ result_mav[,1],
                  lwd=6,
                  type="l",
                  pch=20,
                  col="#990000")
