...
# ' @param GM = NULL, Genetic Map to provide coordinates for GD, a data.frame with the columns "SNP", "Chromosome", "Position".
...
# ' "G",
# ' "GD",
# ' "GI", (= GM) a data.frame with column names "SNP", "Chromosome", and "Position".
# ' "GT", a matrix with taxa in one column.
# ' "hasGenotype", a logical
# ' "genoFormat", string indicating input genotype format
# ' "KI", data.frame, sample by sample kinship matrix
# ' "PC", 
# ' "byFile", logical
# ' "fullGD", logical
# ' "Timmer", 
# ' "Memory",
# ' "SNP.QTN", returned when kinship.algorithm=="SUPER" 
# ' "chor_taxa", a character vector of chromosome names. #[1] "1" "2" "3" "4" "5" "6"
# '
# '
# '
# ' @author Zhiwu Zhang
...
#print("Genotyping: numericalization, sampling kinship, PCs and much more...")
  ...
  # If we have genotypes in RAM (G or GD), set byData = TRUE.
  byData = !is.null(G) | !is.null(GD)
  # If we have files, instead of data in RAM, set byFile = TRUE.
  ...
  # In one of the 3 situations, calculate KI with the algorithm specified, 
  # otherwise skip cit by setting algorithm to "SUPER"
  kinship.algorithm.save = kinship.algorithm
  kinship.algorithm = "SUPER"
  # Normal, no top or bottom.
  # Note: first mention of sangwich top and bottom in GAPIT.Genotype()
  ...
  if( !is.null(GP) & !is.null(sangwich.top) ){
    stop("GAPIT Says: You provided GP. You can not specify sangwich.top")
  }
  ...
  if( !SNP.test & is.null(KI) & !byData & !byFile ){
    stop("GAPIT says: For SNP.test option, please input either use KI or use genotype")
  }
  ...
  if( is.null(file.total) & !byData & byFile){
    stop("GAPIT says: Number of file should be provided: >=1")
  }
  if( !is.null(G) & !is.null(GD) ){
    stop("GAPIT says: Both hapmap and EMMA format exist, choose one only.")
  }
  ...
  # GD = genotypes in 'numeric' format, first column is sample names (taxa),
  ...
  # If Hapmap format, convert to numeric
  if( !is.null(G) ){
    ...
    # hm is a list.
    # hm$GT = matrix, one column of taxa names
    # hm$GD = genotypes, sample by variant matrix
    # hm$GI = data.frame, SNP, Chromosome, Position.
...
#print(unique(GI[,2]))
    ...
    for (file in file.from:numFileUsed) {
      ...
      while( numSNP == file.fragment ) { #this is problematic if the read end at the last line
        ...
        myFRG = GAPIT.Fragment(
           ...
           file.total=file.total,file.G=file.G,file.Ext.G=file.Ext.G,
           seed=seed, SNP.fraction=SNP.fraction, SNP.effect=SNP.effect,
           SNP.impute=SNP.impute,genoFormat=genoFormat,
           file.GD=file.GD,file.Ext.GD=file.Ext.GD,file.GM=file.GM,
  ...
  # modified by Jiabo in 20190927. 
  # sorted number of chrom by numeric and character
  ...
  if(!is.null(GD)){ 
    ...
    ss = apply(GD, 2, sum)
    maf = apply( cbind( 0.5 * ss / (nrow(GD)), 1 - 0.5 * ss / (nrow(GD)) ), 1, min)
    #print(max(maf))
    #print(min(maf))
      ...
      if( nrow(KI) < 2000 ) {
        ...
        if( file.output ) {
          ...
          gplots::heatmap.2(
            ...
            cexRow = 0.2,
            cexCol = 0.2,
            col = rev( grDevices::heat.colors(256) ),
            scale="none",
            symkey=FALSE,
            trace="none"
          )
        ...
        if(!is.null(NJtree.group)&file.output){
          for(tr in 1:length(NJtree.type)) {
            ...
            graphics::legend(
              "topright",
              legend = paste(c("Total individuals is: ","Cluster method: ","Group number: "), 
                             Optimum[c(1:3)],
                             sep=""),
              lty = 0,
              cex = 1.3,
  ...
  # Create kinship matrix (KI) from genotype if not provided
  if( is.null(KI) & (!is.null(GD) | !is.null(GK)) & !kinship.algorithm %in% c("FarmCPU","Blink","MLMM")) {
    ...
    if( kinship.algorithm == "EMMA" ){
      ...
      if( length( which( is.na( half.thisGD ) ) ) > 0 ) {
        print("Substituting missing values with heterozygote for kinship matrix calculation....")
        half.thisGD[which(is.na(half.thisGD))] = 1
      }
      theKin = emma.kinship(
        snps = t( as.matrix( 0.5 * thisGD ) ),
        method ="additive",
        use = "all"
      )
    }
    if( kinship.algorithm == "Loiselle" ) {
      theKin = GAPIT.kinship.loiselle(
        snps = t( as.matrix( 0.5 * thisGD) ),
        method = "additive",
        use = "all"
      )
    ...
    if( kinship.algorithm == "Separation" ){
      ...
      theKin = GAPIT.kinship.separation(
        PCs = thePCA$PCs,
        EV = thePCA$EV,
        nPCs = PCA.total
      )
    ...
    if( !is.null( theKin ) ) {
      ...
      if( length(GT) < 2000 & file.output ){
        ...
        if ( !is.null( NJtree.group ) ) {
          ...
          for( tr in 1:length( NJtree.type ) ) {
            ...
            graphics::legend("topright",legend=paste(c("Total individuals is: ","Group method: ","Group number: "), Optimum[c(1:3)], sep=""),lty=0,cex=1.3,bty="n",bg=graphics::par("bg"))
            grDevices::dev.off()
          }
          # print(Optimum)   
      ...
      if( file.output ) {
        utils::write.table(KI, paste("GAPIT.Kin.",kinship.algorithm,".csv",sep=""), quote = FALSE, sep = ",", row.names = FALSE,col.names = FALSE)
      }
  ...
  if( !is.null(GLD) & file.output ){
    ...
    if( nrow(GLD) > 1 ) {
      ...
      LDsnp = genetics::makeGenotypes(
        hapmapgeno,
        sep = "",
        method = genetics::as.genotype
      )   #This need to be converted to genotype object
      print("Caling LDheatmap...")
      #pdf(paste("GAPIT.LD.pdf",sep=""), width = 12, height = 12)
      grDevices::pdf(
        paste("GAPIT.LD.chromosom", 
              LD.chromosome,
              "(",
