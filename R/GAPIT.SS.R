...
# print(DP$kinship.algorithm)
    ...
    if(DP$file.output){
      utils::write.csv( Pred, 
                        paste("GAPIT.", DP$kinship.algorithm,".Pred.result.csv",sep=""),
                        row.names = FALSE,
                        col.names = TRUE)
    }
...
#						             SNP.CV=SNP.CV,
  ...
  return (
    list(
      GWAS=GWAS,Pred=Pred,FDR=myPower$FDR,Power=myPower$Power,
